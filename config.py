DELAY = (10, 30)                     # Диапазон рандомной задержки между задачами
WAL_AMOUNT_FOR_STAKE = (1.00, 1.50)  # Диапазон рандомного кол-ва WAL токенов для стейкинга каждому валидатору (минимум 1)
MAX_VALIDATORS = 2                   # Количество валидаторов для стейкинга одного аккаунта за один запуск софта (максимум 25)
MAX_CONCURRENT_TASKS = 10            # Одновременно выполняемых аккаунтов
RANGE_OF_ACCOUNTS = (1,)             # Диапазон аккаунтов для выполнения задач, от начала до конца -> (1,)
CHANCE_FOR_MINT = 50                 # Шанс в процентах для минта nft на Flatland (от 0 до 100)
